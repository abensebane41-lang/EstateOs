...
export default async function PropertiesPage({ searchParams }: PropertiesPageProps) {
  ...
  return (
    <div>
      <PageHeader
        title={t("propertiesTable")}
        description={`${t("propertiesSubtitle")}${total > 0 ? ` — ${total} ${tProperty("type")}` : ""}`}
        action={
      ...
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <form action="/dashboard/properties" method="get" className="flex gap-2">
          ...
          <input
            type="text"
            name="search"
            placeholder={t("searchInProperties")}
            defaultValue={search}
      ...
      {properties.length === 0 ? (
        <div className="rounded-xl border border-border bg-white">
          <EmptyState
            ...
            description={search ? t("noSearchResults") : t("noPropertiesHint")}
            action={
              !search ? (
                <a href="/dashboard/properties/new">
                  <Button>
