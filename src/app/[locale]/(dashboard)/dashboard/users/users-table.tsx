...
export function UsersTable({ initialUsers }: { initialUsers: User[] }) {
  ...
  const roleLabels: Record<string, string> = {
    AGENCY_OWNER: t("roleOwner"),
    AGENCY_AGENT: t("roleAgent"),
  ...
  async function handleDelete(id: string) {
    if (!confirm(tConfirmation("deleteConfirm"))) return;
    await deleteUser(id);
  ...
  return (
    <div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-surface-secondary">
                <th className="text-right px-4 py-3 font-medium text-text-secondary">{t("tableUserName")}</th>
                <th className="text-right px-4 py-3 font-medium text-text-secondary">{t("tableUserEmail")}</th>
                <th className="text-right px-4 py-3 font-medium text-text-secondary">{t("tableUserRole")}</th>
                <th className="text-right px-4 py-3 font-medium text-text-secondary">{t("tableUserPhone")}</th>
                <th className="text-right px-4 py-3 font-medium text-text-secondary">{t("tableUserDate")}</th>
                <th className="text-right px-4 py-3 font-medium text-text-secondary">{tCommon("actions")}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {users.length === 0 ? (
                <tr><td colSpan={6} className="px-4 py-12 text-center text-text-secondary">{t("noUsers")}</td></tr>
              ) : (
                users.map((user) => (
                  <tr key={user.id} className="hover:bg-surface-secondary/50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-text-secondary" dir="ltr">{user.phone ?? tCommon("none")}</td>
      ...
      <Dialog open={addDialog} onOpenChange={setAddDialog}>
        <DialogContent>
          <DialogHeader><DialogTitle>{t("newUser")}</DialogTitle></DialogHeader>
          <div className="space-y-4">
            {error && <div className="rounded-lg bg-error/10 p-3 text-sm text-error">{error}</div>}
            <div className="space-y-2">
              <Label>{t("userName")}</Label>
              <Input value={formData.name} onChange={(e) => setFormData((p) => ({ ...p, name: e.target.value }))} placeholder={t("userNamePlaceholder")} />
            </div>
            <div className="space-y-2">
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setAddDialog(false)}>{tCommon("cancel")}</Button>
            <Button onClick={handleAdd} disabled={loading || !formData.name || !formData.email || !formData.password}>
              {loading ? t("createUserLoading") : t("createUser")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!successInfo} onOpenChange={() => setSuccessInfo(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("userAdded")}</DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <p className="text-sm text-text-secondary">{t("userAddedDesc")}</p>
            <div className="rounded-xl border border-border bg-surface-secondary p-4 space-y-2">
              ...
              <div className="flex justify-between">
                <span className="text-xs text-text-secondary">{t("userEmail")}</span>
                <span className="text-sm font-medium text-text-primary" dir="ltr">{successInfo?.email}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-xs text-text-secondary">{t("userPassword")}</span>
          ...
          <DialogFooter>
            <Button onClick={() => setSuccessInfo(null)}>{tCommon("ok")}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
